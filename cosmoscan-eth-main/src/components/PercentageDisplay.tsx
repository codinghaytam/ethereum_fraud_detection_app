...
const CircularProgressRevert = ({
  ...
  // Color logic based on percentage
  const getProgressColor = (value: number) => {
    if (value <= 30) return "stroke-orange-500";
  ...
  const getTextColor = (value: number) => {
    if (value <= 30) return "text-orange-500"; // Low percentage - green (safe)
    if (value <= 60) return "text-yellow-500"; // Medium percentage - yellow (warning)
    if (value <= 80) return "text-green-500"; // High percentage - orange (caution)
    return "text-green-500"; // Very high percentage - red (danger)
  };
...
export function CircularProgressDemoInvert(props: {
  value: number;
  size?: number;
  strokeWidth?: number;
  circleStrokeWidth?: number;
