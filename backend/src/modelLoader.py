...
ETHERSCAN_NUMERIC_COLUMNS = [
    ...
    'gasUsed', 'txreceipt_status', 'confirmations'
]
...
def predict_address_fraud(ethereum_address, etherscan_api_key, model_path=DEFAULT_MODEL_PATH):
    ...
    try:
        ...
        raw_fetch= transaction_df.copy() if transaction_df is not None else []
        ...
        if transaction_df is None or len(transaction_df) == 0 :
            return {
                ...
                'errorCode': 'NOT_ENOUGH_TRANSACTIONS'
        ...
        prediction_result = {
            ...
            'total_transactions': int(total_transactions),
            'transaction_used': raw_fetch.to_dict('records') if raw_fetch is not None else [],
            'error': None
        }
