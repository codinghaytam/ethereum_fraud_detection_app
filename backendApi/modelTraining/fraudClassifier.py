...
class FraudClassifier(nn.Module):
    def __init__(self, sequence_input_size, static_input_size=0, hidden_size=128, num_layers=3, 
        ...
        # Output layer
        fc_layers.append(nn.Linear(prev_size, num_classes,bias=True))
...
def prepare_data():
    ...
    addresses= pl.concat([addresses.filter(pl.col('FLAG').is_in([0])).sample(n=2179),addresses.filter(pl.col('FLAG').is_in([1]))])  # Ensure addresses with labels are included
...
def train_model(model, train_sequences, train_labels, train_static, val_sequences, val_labels, val_static, epochs=20, batch_size=32):
    ...
    for epoch in range(epochs):
        ...
        for batch in tqdm.tqdm(train_loader, desc=f"Epoch {epoch+1}/{epochs} - Training"):
            ...
            labels_onehot = torch.zeros(labels.size(0), 2).to(labels.device)
...
model = train_model(model, train_sequences, train_labels, train_static, val_sequences, val_labels, val_static, epochs=10, batch_size=32)
...
#saving model
torch.save(model.state_dict(), 'fraud_classifier_WithBCEWLoss.pth')
