...
data = pl.read_csv("../data/transaction_dataset.csv")
data = pl.concat([data.filter(pl.col("FLAG") == 1), data.filter(pl.col("FLAG") == 0).sample(n=2800)])
...
X_train = train_split.drop(["Index","FLAG",""], axis=1)
y_train = train_split["FLAG"]
X_test = test_split.drop(["Index","FLAG",""], axis=1)
y_test = test_split["FLAG"]
